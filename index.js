...
try {
    ...
    class Music {
      constructor(client, options) {
        ...
        this.play = {
          ...
          run: "playFunction",
          alt: (options && options.play && options.play.alt) || ["p"],
        ...
        this.join = {
          enabled: (options.join == undefined ? true : (options.join && typeof options.join.enabled !== 'undefined' ? options.join && options.join.enabled : true)),
          run: "joinFunction",
          alt: (options && options.join && options.join.alt) || [],
          help: (options && options.join && options.join.help) || "Joined the voice channel.",
          name: (options && options.join && options.join.name) || "join",
        ...
        this.queue = {
          ...
          run: "queueFunction",
          alt: (options && options.queue && options.queue.alt) || ["q"],
        ...
        this.volume = {
          ...
          run: "volumeFunction",
          alt: (options && options.volume && options.volume.alt) || ["v"],
        ...
        this.skip = {
          ...
          run: "skipFunction",
          alt: (options && options.skip && options.skip.alt) || ["s"],
        ...
        this.embedColor = (options && options.embedColor) || 'RANDOM';
        ...
        this.anyoneCanJoin = (options && typeof options.anyoneCanJoin !== 'undefined' ? options && options.anyoneCanJoin : true);
    ...
    musicbot.joinFunction = (msg, suffix) => {
      if (musicbot.isAdmin(msg.member) || musicbot.anyoneCanJoin === true) {
        if (!msg.member.voiceChannel) return msg.channel.send(musicbot.note('fail', `You're not in a voice channel.`));

        msg.member.voiceChannel.join()
        msg.channel.send(musicbot.note('note', 'Successfully join the voice channel.'));
      } else {
        const chance = Math.floor((Math.random() * 100) + 1);
        if (chance <= 10) return msg.channel.send(musicbot.note('fail', `I'm afraid I can't let you do that, ${msg.author.username}.`))
        else return msg.channel.send(musicbot.note('fail', 'Sorry, you\'re not allowed to do that.'));
              ...
              setTimeout(() => {
                ...
                if (queue.songs.length > 0) {
                  if (loop == "none" || loop == null) {
                    queue.songs.shift();
                    musicbot.updatePositions(queue, msg.guild.id).then(res => {
